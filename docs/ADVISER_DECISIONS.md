...
| Audio coaching timing | The paper alternates between post-game analysis and “live audio coaching.” | No live audio coaching. The former dashboard audio widget is only in a sample-data preview. | Pending |
...
| Shot classification | Capstone 1 requires shot classes, including a “trajectory arc height” input. Ground-plane mapping from one fixed camera cannot measure ball height. | Shot classification is `not_computed`; there is no ball tracker or validated classifier. | Pending: remove, defer, or specify a feasible measurement setup and evaluation protocol. |
